...
function redirect_to($page, $params = []){$query = array_merge(['page' => $page], $params);header('Location: ?' . http_build_query($query));exit;}
...
function table_column_exists($table,$column){
    try{
        $dbName = DB_NAME;
        return (int)val("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME=? AND COLUMN_NAME=?", [$dbName,$table,$column]) > 0;
    } catch(Exception $e){ return false; }
}
function ensure_runtime_schema(){
    try{
        $adds=[
            'idp_area' => "ALTER TABLE final_results ADD COLUMN idp_area VARCHAR(120) DEFAULT NULL",
            'idp_plan' => "ALTER TABLE final_results ADD COLUMN idp_plan VARCHAR(500) DEFAULT NULL",
            'idp_target' => "ALTER TABLE final_results ADD COLUMN idp_target VARCHAR(255) DEFAULT NULL",
            'idp_status' => "ALTER TABLE final_results ADD COLUMN idp_status VARCHAR(30) DEFAULT 'belum_dibuat'",
            'idp_updated_by' => "ALTER TABLE final_results ADD COLUMN idp_updated_by INT NULL",
            'idp_updated_at' => "ALTER TABLE final_results ADD COLUMN idp_updated_at DATETIME NULL"
        ];
        foreach($adds as $col=>$sql){
            if(!table_column_exists('final_results',$col)) db()->exec($sql);
        }
    } catch(Exception $e){ /* schema migration is best-effort for local demo */ }
}
function selected_period_id(){ return isset($_GET['period_id']) && $_GET['period_id']!=='' ? (int)$_GET['period_id'] : null; }
function selected_department_id(){ return isset($_GET['department_id']) && $_GET['department_id']!=='' ? (int)$_GET['department_id'] : null; }
function selected_period(){
    $pid=selected_period_id();
    if($pid) return one("SELECT * FROM evaluation_periods WHERE period_id=?", [$pid]);
    return null;
}
function render_period_filter($extra=[]){
    $periods=all("SELECT * FROM evaluation_periods ORDER BY start_date DESC, period_id DESC");
    $selected=selected_period_id();
    echo '<form method="get" class="filter-bar"><input type="hidden" name="page" value="'.h($_GET['page'] ?? 'reports').'">';
    foreach($extra as $name=>$value) echo '<input type="hidden" name="'.h($name).'" value="'.h($value).'">';
    echo '<select name="period_id"><option value="">Periode aktif / semua periode</option>';
    foreach($periods as $p){
        echo '<option value="'.$p['period_id'].'" '.((int)$p['period_id']===$selected?'selected':'').'>'.h($p['period_name']).' · '.h($p['period_status']).'</option>';
    }
    echo '</select><button class="btn primary">Terapkan Filter</button></form>';
}
function threshold_label($score){
    $score=(float)$score;
    if($score >= 4.50) return 'High Potential';
    if($score >= 3.50) return 'Solid Performer';
    if($score > 0) return 'Need Development';
    return 'Belum Dinilai';
}
function threshold_badge($score){
    $label=threshold_label($score);
    $cls=$label==='High Potential'?'good':($label==='Solid Performer'?'neutral':'risk');
    return '<span class="badge '.$cls.'">'.h($label).'</span>';
}
function idp_status_badge($status){
    $status=$status ?: 'belum_dibuat';
    $label=['belum_dibuat'=>'Belum dibuat','draft'=>'Draft','final'=>'Final'][$status] ?? $status;
    $cls=$status==='final'?'good':($status==='draft'?'warn':'risk');
    return '<span class="badge '.$cls.'">'.h($label).'</span>';
}
function assessment_status_for($evaluator_id,$evaluatee_id,$period_id,$type){
    $row=one("SELECT assessment_status FROM assessments WHERE evaluator_id=? AND evaluatee_id=? AND period_id=? AND assessment_type=?", [$evaluator_id,$evaluatee_id,$period_id,$type]);
    return $row['assessment_status'] ?? 'belum_dikerjakan';
}
function assessment_status_badge($status){
    if($status==='submitted') return '<span class="badge good">Sudah dikirim</span>';
    if($status==='draft') return '<span class="badge warn">Draft</span>';
    return '<span class="badge risk">Belum dikerjakan</span>';
}
function pending_assessment_count($u,$period){
    if(!$period) return 0;
    $count=0;
    foreach(assessment_targets($u,$period) as $t){
        $st=assessment_status_for($u['employee_id'],$t['evaluatee_id'],$period['period_id'],$t['type']);
        if($st!=='submitted') $count++;
    }
    return $count;
}
function should_show_deadline_popup($u){
    if(!$u || in_array($u['role_name'], ['admin_hr','manajemen'], true)) return null;
    $period=active_period();
    if(!$period || empty($period['end_date'])) return null;
    $today=new DateTime(date('Y-m-d'));
    $end=new DateTime($period['end_date']);
    $diff=(int)$today->diff($end)->format('%r%a');
    $pending=pending_assessment_count($u,$period);
    if($diff>=0 && $diff<=3 && $pending>0) return ['period'=>$period,'days'=>$diff,'pending'=>$pending];
    return null;
}
function primary_gap_info($employee_id,$period_id){
    $weak=one("SELECT av.akhlak_name, ROUND(AVG(ad.score),2) avg_score
               FROM assessments a
    ...
    if(!$weak) return ['area'=>'Belum ada data','score'=>0,'text'=>'Belum ada data assessment lengkap.'];
    return ['area'=>$weak['akhlak_name'],'score'=>(float)$weak['avg_score'],'text'=>'Gap utama pada nilai '.$weak['akhlak_name'].' dengan skor '.score_fmt($weak['avg_score']).'.'];
}
function department_gap_rows($period_id=null){
    $where=[]; $params=[];
    if($period_id){ $where[]='fr.period_id=?'; $params[]=$period_id; }
    $sqlWhere=$where?'WHERE '.implode(' AND ',$where):'';
    return all("SELECT d.department_id, d.department_name, ROUND(AVG(fr.final_score),2) avg_score,
                       SUM(CASE WHEN fr.final_score>0 AND fr.final_score<3.50 THEN 1 ELSE 0 END) need_dev,
                       COUNT(*) total_employee
                FROM final_results fr
                JOIN employees e ON e.employee_id=fr.employee_id
                JOIN departments d ON d.department_id=e.department_id
                $sqlWhere
                GROUP BY d.department_id,d.department_name ORDER BY avg_score ASC", $params);
}
function employee_threshold_rows($period_id=null){
    $where=["fr.final_score > 0 AND fr.final_score < 3.50"]; $params=[];
    if($period_id){ $where[]='fr.period_id=?'; $params[]=$period_id; }
    return all("SELECT fr.*, e.employee_name, e.employee_nik, d.department_name, ep.period_name
                FROM final_results fr
                JOIN employees e ON e.employee_id=fr.employee_id
                JOIN departments d ON d.department_id=e.department_id
                JOIN evaluation_periods ep ON ep.period_id=fr.period_id
                WHERE ".implode(' AND ',$where)."
                ORDER BY fr.final_score ASC", $params);
}
...
function report_rows($u,$filters=[]){
    $where=[]; $params=[];
    if(($u['role_name'] ?? '')==='staff'){
        $where[]='fr.employee_id=?'; $params[]=$u['employee_id'];
    } elseif(($u['role_name'] ?? '')==='atasan'){
        $where[]='(e.supervisor_id=? OR e.employee_id=?)'; $params[]=$u['employee_id']; $params[]=$u['employee_id'];
    }
    $pid = $filters['period_id'] ?? selected_period_id();
    if($pid){ $where[]='fr.period_id=?'; $params[]=(int)$pid; }
    $dept = $filters['department_id'] ?? selected_department_id();
    if($dept && can_manage_all($u)){ $where[]='e.department_id=?'; $params[]=(int)$dept; }
    $sqlWhere=$where?'WHERE '.implode(' AND ',$where):'';
    return all("SELECT fr.*, e.employee_nik, e.employee_name, e.department_id, d.department_name, p.position_name, ep.period_name, ep.period_status
                FROM final_results fr
                JOIN employees e ON e.employee_id=fr.employee_id
                JOIN departments d ON d.department_id=e.department_id
                JOIN positions p ON p.position_id=e.position_id
                JOIN evaluation_periods ep ON ep.period_id=fr.period_id
                $sqlWhere
                ORDER BY ep.start_date DESC, fr.final_score DESC", $params);
}
...
function assessment_targets($u,$period){
    ...
    // Admin HR dan Manajemen tidak mengisi assessment pada rancangan final.
    if(in_array($role, ['admin_hr','manajemen'], true)) return [];

    // Self assessment tersedia untuk Karyawan dan Atasan pada periode aktif.
    $targets[]=[
        'evaluatee_id'=>$eid,
    ...
    if(in_array($role, ['staff','atasan'], true) && !empty($u['supervisor_id'])){
...
if(isset($_GET['download'])){
    ...
    $rows=report_rows($u,$_GET); $fmt=$_GET['download'];
    if($fmt==='csv'){
...
if($_SERVER['REQUEST_METHOD']==='POST' && $u){
    $action=$_POST['action'] ?? '';
    try{
        if($action==='sync_hris' && can_admin($u)){
            $total=(int)val("SELECT COUNT(*) FROM employees WHERE employee_status='active'");
            create_notification(null,'Sinkronisasi Data HRIS Berhasil','Data master karyawan berhasil disinkronkan. Total karyawan aktif: '.$total.'.','employee');
            flash('Sinkronisasi data HRIS berhasil. Total karyawan aktif: '.$total.'.');
            add_audit('Sinkronisasi Data HRIS');
            redirect_to('employees');
        }
        if($action==='save_idp' && role_allows($u,['atasan'])){
            $result_id=(int)($_POST['result_id'] ?? 0);
            $row=one("SELECT fr.*, e.employee_name, e.supervisor_id FROM final_results fr JOIN employees e ON e.employee_id=fr.employee_id WHERE fr.result_id=?", [$result_id]);
            if(!$row) throw new Exception('Data hasil evaluasi tidak ditemukan.');
            if((int)$row['supervisor_id'] !== (int)$u['employee_id']) throw new Exception('Atasan hanya dapat mengisi IDP untuk bawahan langsung.');
            $area=trim($_POST['idp_area'] ?? '');
            $plan=trim($_POST['idp_plan'] ?? '');
            $target=trim($_POST['idp_target'] ?? '');
            $status=$_POST['idp_status'] ?? 'draft';
            if(!in_array($status,['belum_dibuat','draft','final'],true)) $status='draft';
            if($area==='' || $plan==='' || $target==='') throw new Exception('Area pengembangan, rencana IDP, dan target perbaikan wajib diisi.');
            q("UPDATE final_results SET idp_area=?, idp_plan=?, idp_target=?, idp_status=?, idp_updated_by=?, idp_updated_at=NOW() WHERE result_id=?", [$area,$plan,$target,$status,$u['user_id'],$result_id]);
            create_notification(null,'IDP Diperbarui',$u['employee_name'].' memperbarui IDP untuk '.$row['employee_name'].'.','report');
            flash('IDP berhasil disimpan.'); add_audit('Isi/Edit IDP '.$row['employee_name']); redirect_to('reports', ['period_id' => $period_id]);
        }
        ...
        if(in_array($action,['send_notification','save_notification'],true) && can_admin($u)){
            $target=($_POST['user_id'] ?? '')==='' ? null : (int)$_POST['user_id'];
            $title=trim($_POST['title'] ?? '');
            $message=trim($_POST['message'] ?? '');
            $type=$_POST['notification_type'] ?? 'info';
            if($title==='' || $message==='') throw new Exception('Judul dan isi notifikasi wajib diisi.');
            $nid=(int)($_POST['notification_id'] ?? 0);
            if($nid>0){
                q("UPDATE notifications SET user_id=?, title=?, message=?, notification_type=? WHERE notification_id=?", [$target,$title,$message,$type,$nid]);
                flash('Notifikasi berhasil diperbarui.'); add_audit('Edit notifikasi');
            } else {
                create_notification($target,$title,$message,$type);
                flash('Notifikasi berhasil dibuat.'); add_audit('Membuat notifikasi');
            }
            redirect_to('notifications');
        }
        if($action==='delete_notification' && can_admin($u)){
            q("DELETE FROM notifications WHERE notification_id=?", [(int)$_POST['notification_id']]);
            flash('Notifikasi berhasil dihapus.'); add_audit('Hapus notifikasi'); redirect_to('notifications');
        }
        if($action==='mark_read'){
...
function layout_start($u,$title){
    ...
    foreach($nav as $p=>$label){
        ...
        if($p==='assessment' && !role_allows($u,['staff','atasan'])) continue;
        if($p==='talent' && !role_allows($u,['manajemen'])) continue;
    ...
    $reminder=should_show_deadline_popup($u);
    if($reminder){
        echo '<div class="deadline-popup"><div><b>Reminder Tenggat Waktu</b><p>Periode '.h($reminder['period']['period_name']).' akan berakhir dalam '.h($reminder['days']).' hari. Masih ada '.h($reminder['pending']).' assessment yang belum diselesaikan.</p><a class="btn primary" href="?page=assessment">Lengkapi Assessment</a></div></div>';
    }
}
...
if($page==='dashboard'){
    ...
    if($u['role_name']==='staff'){
        ...
        $topRows=all("SELECT e.employee_name, d.department_name, fr.final_score FROM final_results fr JOIN employees e ON e.employee_id=fr.employee_id JOIN departments d ON d.department_id=e.department_id WHERE fr.period_id=? ORDER BY fr.final_score DESC LIMIT 5", [$pid]);
        echo '<div class="metric-grid"><div class="metric"><span>Total Karyawan Aktif</span><b>'.$totalEmp.'</b></div><div class="metric"><span>Assessment Submitted</span><b>'.$submitted.'</b></div><div class="metric"><span>Rata-rata Organisasi</span><b>'.score_fmt($avg).'</b></div><div class="metric"><span>Top Skor Karyawan</span><b>'.score_fmt($top['final_score'] ?? 0).'</b><small>'.h($top['employee_name'] ?? '-').'</small></div></div>';
        echo '<div class="grid-2"><div class="panel"><h2>Monitoring Budaya AKHLAK</h2><p class="muted">Nilai rata-rata AKHLAK organisasi untuk melihat area budaya yang perlu diperkuat.</p>'; render_akhlak_bars(akhlak_scores(null,$pid)); echo '</div><div class="panel"><h2>Distribusi Departemen</h2>'; render_department_scores($pid); echo '</div></div>';
        echo '<div class="panel"><h2>Top Skor Karyawan</h2><div class="table-wrap"><table><tr><th>Nama</th><th>Departemen</th><th>Final Score</th><th>Kategori</th></tr>';
        foreach($topRows as $tr){ echo '<tr><td><b>'.h($tr['employee_name']).'</b></td><td>'.h($tr['department_name']).'</td><td>'.score_fmt($tr['final_score']).'</td><td>'.threshold_badge($tr['final_score']).'</td></tr>'; }
        echo '</table></div></div>';
    }
...
function render_notifications_list($rows){
    ...
    foreach($rows as $n){
        echo '<div><b>'.h($n['title']).'</b> '.badge($n['is_read']?'read':'unread').'<p>'.h($n['message']).'</p><small>'.h($n['notification_type']).' · '.h($n['created_at']).'</small><div class="actions">';
        if(!$n['is_read']) echo '<form method="post" style="display:inline"><input type="hidden" name="action" value="mark_read"><input type="hidden" name="notification_id" value="'.$n['notification_id'].'"><button class="btn small ghost">Tandai Dibaca</button></form>';
        if(can_admin($u)){
            echo '<a class="btn small primary" href="?page=notifications&edit_id='.$n['notification_id'].'">Edit</a> ';
            echo '<form method="post" style="display:inline" onsubmit="return confirm(\'Hapus notifikasi ini?\')"><input type="hidden" name="action" value="delete_notification"><input type="hidden" name="notification_id" value="'.$n['notification_id'].'"><button class="btn small ghost">Hapus</button></form>';
        }
        echo '</div></div>';
    }
    echo '</div>';
...
if($page==='employees' && can_admin($u)){
    ...
    echo '<div class="panel"><h2>Sinkronisasi Data HRIS</h2><p class="muted">Fitur ini digunakan untuk menyegarkan data master karyawan sebagai simulasi sinkronisasi dari HRIS. Aktivitas akan tercatat di audit trail.</p><form method="post"><input type="hidden" name="action" value="sync_hris"><button class="btn primary">Sinkronisasi Data HRIS</button></form></div>';
...
if($page==='assessment'){
    if(!role_allows($u,['staff','atasan'])){ flash('Menu Isi Assessment hanya tersedia untuk Karyawan dan Atasan.','error'); redirect_to('dashboard'); }
    ...
    echo '<div class="panel"><h2>Target Penilaian Periode Aktif</h2><p class="muted">Setiap target menampilkan status pengerjaan: belum dikerjakan, draft, atau sudah dikirim.</p><div class="cards">';
    foreach($targets as $i=>$t){
        $st=assessment_status_for($u['employee_id'],$t['evaluatee_id'],$period['period_id'],$t['type']);
        echo '<a class="mini-card" href="?page=assessment&target='.$t['evaluatee_id'].'&type='.$t['type'].'"><span>'.h(type_label($t['type'])).'</span><b>'.h($t['name']).'</b><p>'.h($t['reason']).'</p>'.assessment_status_badge($st).'</a>';
    }
    echo '</div></div>';
    ...
    $currentStatus=assessment_status_for($u['employee_id'],$target,$period['period_id'],$type);
    echo '<div class="panel"><h2>Form '.h(type_label($type)).' untuk '.h($selectedTarget['name']).'</h2><p>Status: '.assessment_status_badge($currentStatus).'</p><form method="post"><input type="hidden" name="action" value="submit_assessment"><input type="hidden" name="evaluatee_id" value="'.h($target).'"><input type="hidden" name="assessment_type" value="'.h($type).'">';
...
if($page==='reports' && can_report($u)){
    layout_start($u,'Report & IDP');
    $pid=selected_period_id();
    $rows=report_rows($u,$_GET);
    $querySuffix=$pid ? '&period_id='.$pid : '';
    echo '<div class="panel"><h2>Filter Periode Report</h2><p class="muted">Gunakan filter periode untuk melihat riwayat penilaian dan IDP berdasarkan periode evaluasi.</p>';
    render_period_filter();
    echo '</div>';

    if($u['role_name']==='atasan' && isset($_GET['idp_result_id'])){
        $rid=(int)$_GET['idp_result_id'];
        $idpRow=one("SELECT fr.*, e.employee_name, e.supervisor_id, ep.period_name
                     FROM final_results fr
                     JOIN employees e ON e.employee_id=fr.employee_id
                     JOIN evaluation_periods ep ON ep.period_id=fr.period_id
                     WHERE fr.result_id=?", [$rid]);
        if($idpRow && (int)$idpRow['supervisor_id']===(int)$u['employee_id']){
            $gap=primary_gap_info($idpRow['employee_id'],$idpRow['period_id']);
            echo '<div class="panel"><h2>Isi / Edit IDP Bawahan</h2><p class="muted">IDP diisi oleh atasan untuk bawahan langsung berdasarkan gap analysis periode terpilih.</p><form class="form-grid" method="post"><input type="hidden" name="action" value="save_idp"><input type="hidden" name="result_id" value="'.$idpRow['result_id'].'"><input value="Karyawan: '.h($idpRow['employee_name']).'" readonly><input value="Periode: '.h($idpRow['period_name']).'" readonly><input name="idp_area" placeholder="Area pengembangan" value="'.h($idpRow['idp_area'] ?: $gap['area']).'" required><input value="Skor area gap: '.score_fmt($gap['score']).'" readonly><textarea name="idp_plan" placeholder="Rencana pengembangan / rekomendasi IDP" required>'.h($idpRow['idp_plan'] ?: idp_recommendation($idpRow['final_score'])).'</textarea><textarea name="idp_target" placeholder="Target perbaikan" required>'.h($idpRow['idp_target'] ?: 'Meningkatkan skor area '.$gap['area'].' menjadi minimal 3.50 pada periode berikutnya.').'</textarea><select name="idp_status"><option value="draft" '.(($idpRow['idp_status']??'')==='draft'?'selected':'').'>Draft</option><option value="final" '.(($idpRow['idp_status']??'')==='final'?'selected':'').'>Final</option></select><button class="btn primary full">Simpan IDP</button><a class="btn ghost full" href="?page=reports'.$querySuffix.'">Batal</a></form></div>';
        }
    }

    echo '<div class="panel"><h2>Export Laporan</h2><p class="muted">Karyawan melihat report pribadi, Atasan melihat tim/bawahan, Admin HR dan Manajemen melihat data organisasi sesuai hak akses.</p><div class="actions"><a class="btn primary" href="?page=report_print'.$querySuffix.'" target="_blank">Download PDF</a><a class="btn ghost" href="?download=excel'.$querySuffix.'">Download Excel</a><a class="btn ghost" href="?download=csv'.$querySuffix.'">Download CSV</a></div></div>';

    if($u['role_name']==='manajemen'){
        $deptRows=department_gap_rows($pid);
        echo '<div class="grid-2"><div class="panel"><h2>Gap Antar Departemen</h2><p class="muted">Departemen diurutkan dari rata-rata skor terendah untuk membantu prioritas penguatan budaya AKHLAK.</p><div class="table-wrap"><table><tr><th>Departemen</th><th>Rata-rata Skor</th><th>Pegawai Rendah</th><th>Total Data</th><th>Rekomendasi</th></tr>';
        foreach($deptRows as $d){ echo '<tr><td><b>'.h($d['department_name']).'</b></td><td>'.score_fmt($d['avg_score']).'</td><td>'.h($d['need_dev']).'</td><td>'.h($d['total_employee']).'</td><td>'.h($d['avg_score']<3.5?'Prioritas program pengembangan departemen.':'Monitoring berkala dan pertahankan performa.').'</td></tr>'; }
        echo '</table></div></div>';
        $thresholdRows=employee_threshold_rows($pid);
        echo '<div class="panel"><h2>Pegawai Threshold / Perlu Perhatian</h2><p class="muted">Pegawai dengan final score di bawah 3.50 ditandai sebagai prioritas monitoring dan IDP.</p><div class="table-wrap"><table><tr><th>Nama</th><th>Departemen</th><th>Periode</th><th>Final Score</th><th>Status</th></tr>';
        if(!$thresholdRows) echo '<tr><td colspan="5" class="muted">Tidak ada pegawai di bawah threshold.</td></tr>';
        foreach($thresholdRows as $r){ echo '<tr class="risk-row"><td><b>'.h($r['employee_name']).'</b></td><td>'.h($r['department_name']).'</td><td>'.h($r['period_name']).'</td><td>'.score_fmt($r['final_score']).'</td><td>'.threshold_badge($r['final_score']).'</td></tr>'; }
        echo '</table></div></div></div>';
    }

    echo '<div class="panel"><h2>Hasil Evaluasi, Gap Analysis, dan IDP</h2><div class="table-wrap"><table><tr><th>NIK</th><th>Nama</th><th>Departemen</th><th>Periode</th><th>Self</th><th>Peer</th><th>Bawahan</th><th>Atasan</th><th>Final</th><th>Kategori</th><th>Gap Nilai</th><th>Gap Analysis</th><th>Status IDP</th><th>IDP / Action</th></tr>';
    foreach($rows as $r){
        $gap=primary_gap_info($r['employee_id'],$r['period_id']);
        echo '<tr '.(((float)$r['final_score']>0 && (float)$r['final_score']<3.50)?'class="risk-row"':'').'><td>'.h($r['employee_nik']).'</td><td><b>'.h($r['employee_name']).'</b></td><td>'.h($r['department_name']).'</td><td>'.h($r['period_name']).'</td><td>'.score_fmt($r['self_score']).'</td><td>'.score_fmt($r['peer_score']).'</td><td>'.score_fmt($r['subordinate_score']).'</td><td>'.score_fmt($r['supervisor_score']).'</td><td><b>'.score_fmt($r['final_score']).'</b></td><td>'.threshold_badge($r['final_score']).'</td><td><b>'.h($gap['area']).'</b><br><small>'.score_fmt($gap['score']).'</small></td><td>'.h($r['gap_analysis']).'</td><td>'.idp_status_badge($r['idp_status'] ?? 'belum_dibuat').'</td><td>';
        if(!empty($r['idp_plan'])) echo '<b>'.h($r['idp_area']).'</b><br><small>'.h($r['idp_plan']).'</small><br><small>Target: '.h($r['idp_target']).'</small>';
        else echo '<span class="muted">IDP belum dibuat.</span>';
        if($u['role_name']==='atasan' && (int)($r['employee_id']) !== (int)$u['employee_id']) echo '<br><a class="btn small primary" href="?page=reports'.$querySuffix.'&idp_result_id='.$r['result_id'].'">Isi/Edit IDP</a>';
        echo '</td></tr>';
    }
    echo '</table></div></div>';

    if($u['role_name']==='manajemen'){
        echo '<div class="panel"><h2>Rekap IDP Departemen</h2><div class="table-wrap"><table><tr><th>Departemen</th><th>Total IDP Final</th><th>Total IDP Draft</th><th>Belum Dibuat</th></tr>';
        $idpDept=all("SELECT d.department_name,
            SUM(CASE WHEN fr.idp_status='final' THEN 1 ELSE 0 END) final_count,
            SUM(CASE WHEN fr.idp_status='draft' THEN 1 ELSE 0 END) draft_count,
            SUM(CASE WHEN fr.idp_status IS NULL OR fr.idp_status='belum_dibuat' THEN 1 ELSE 0 END) belum_count
            FROM final_results fr JOIN employees e ON e.employee_id=fr.employee_id JOIN departments d ON d.department_id=e.department_id ".($pid?'WHERE fr.period_id='.(int)$pid:'')." GROUP BY d.department_id,d.department_name ORDER BY d.department_name");
        foreach($idpDept as $d){ echo '<tr><td><b>'.h($d['department_name']).'</b></td><td>'.h($d['final_count']).'</td><td>'.h($d['draft_count']).'</td><td>'.h($d['belum_count']).'</td></tr>'; }
        echo '</table></div></div>';
    }
    layout_end(); exit;
}

if($page==='talent' && role_allows($u,['manajemen'])){
    layout_start($u,'Talent Mapping');
    $pid=selected_period_id();
    echo '<div class="panel"><h2>Filter Periode Talent Mapping</h2><p class="muted">Talent mapping dihitung berdasarkan final score pada periode terpilih.</p>';
    render_period_filter();
    echo '</div>';
    $where=[]; $params=[];
    if($pid){ $where[]='fr.period_id=?'; $params[]=$pid; }
    $sqlWhere=$where?'WHERE '.implode(' AND ',$where):'';
    $rows=all("SELECT fr.*, e.employee_name, e.employee_nik, d.department_name, ep.period_name
                FROM final_results fr
                JOIN employees e ON e.employee_id=fr.employee_id
                JOIN departments d ON d.department_id=e.department_id
                JOIN evaluation_periods ep ON ep.period_id=fr.period_id
    ...
    $counts=['High Potential'=>0,'Solid Performer'=>0,'Need Development'=>0,'Belum Dinilai'=>0];
    foreach($rows as $r){ $counts[threshold_label($r['final_score'])]++; }
    echo '<div class="metric-grid"><div class="metric"><span>High Potential</span><b>'.$counts['High Potential'].'</b><small>Skor ≥ 4.50</small></div><div class="metric"><span>Solid Performer</span><b>'.$counts['Solid Performer'].'</b><small>Skor 3.50 - 4.49</small></div><div class="metric"><span>Need Development</span><b>'.$counts['Need Development'].'</b><small>Skor di bawah 3.50</small></div><div class="metric"><span>Total Data</span><b>'.count($rows).'</b><small>Periode terpilih</small></div></div>';
    echo '<div class="panel"><h2>Daftar Talent Mapping</h2><div class="table-wrap"><table><tr><th>NIK</th><th>Nama</th><th>Departemen</th><th>Periode</th><th>Final Score</th><th>Kategori Talent</th><th>Rekomendasi</th></tr>';
    foreach($rows as $r){
        $cat=threshold_label($r['final_score']);
        $rec=$cat==='High Potential'?'Kandidat talent pool dan project leadership.':($cat==='Solid Performer'?'Pertahankan performa dan berikan coaching rutin.':'Prioritas IDP dan monitoring atasan.');
        echo '<tr '.($cat==='Need Development'?'class="risk-row"':'').'><td>'.h($r['employee_nik']).'</td><td><b>'.h($r['employee_name']).'</b></td><td>'.h($r['department_name']).'</td><td>'.h($r['period_name']).'</td><td>'.score_fmt($r['final_score']).'</td><td>'.threshold_badge($r['final_score']).'</td><td>'.h($rec).'</td></tr>';
    }
    echo '</table></div></div>';
    layout_end(); exit;
}
...
if($page==='notifications'){
    ...
    if(can_admin($u)){
        ...
        $edit_id=$_GET['edit_id'] ?? null;
        $edit=null;
        if($edit_id) $edit=one("SELECT * FROM notifications WHERE notification_id=?", [(int)$edit_id]);
        echo '<div class="panel"><h2>'.($edit?'Edit Notifikasi':'Create Notice / Reminder').'</h2><form class="form-grid" method="post"><input type="hidden" name="action" value="save_notification"><input type="hidden" name="notification_id" value="'.($edit?(int)$edit['notification_id']:'').'"><select name="user_id"><option value="">Semua pengguna</option>'; foreach($users as $usr) echo '<option value="'.$usr['user_id'].'" '.($edit && $edit['user_id']==$usr['user_id']?'selected':'').'>'.h($usr['employee_name']).' - '.role_label($usr['role_name']).'</option>'; echo '</select><input name="title" placeholder="Judul notifikasi" required value="'.($edit?h($edit['title']):'').'"><select name="notification_type">';
        foreach(['info','period','reminder','approval','report','employee','assessment'] as $type){ echo '<option value="'.$type.'" '.($edit && $edit['notification_type']===$type?'selected':'').'>'.$type.'</option>'; }
        echo '</select><textarea name="message" placeholder="Isi pesan notifikasi" required>'.($edit?h($edit['message']):'').'</textarea><button class="btn primary full">'.($edit?'Perbarui Notifikasi':'Kirim Notifikasi').'</button>'.($edit?'<a class="btn ghost full" href="?page=notifications">Batal Edit</a>':'').'</form></div>';
    }
